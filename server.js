...
1. Check if ${closestInst ? closestInst.name : “the closest instructor”} has any upcoming days when they’ll already be working near ${clientSuburb}
1. If urgent, speak directly with an instructor about a special arrangement`;
   ...
   : `after lesson ending ${s.windowEarliest.replace(/^(\d+):(\d+)$/, (_, h, m) => { // work back to find the end time of the previous appt return s.windowEarliest; })} at ${s.prevLocation} (${s.travelIn} min drive to client)`;
   ...
   return `Slot ${i + 1}: ${s.instructor} — ${formatDate(s.date)} (${s.dayName}) at ${s.suggestedStart}   ${tierLabel}   Valid window: ${s.windowEarliest}–${s.windowLatest}   Before: ${prevDesc}   After: ${nextDesc}   Day total: ${s.totalApptsThatDay} booking(s) that day   Availability preference: ${s.period} ${baseNote}${notesLine}`;
   }).join(”\n\n”);
...
1. For each slot, explain: the day/time match, where the instructor is coming from, what’s before and after
1. Apply the tier labels appropriately
